...
def processFrame(image):
    ...
    #image = image_.half() if half else image_.float()
    image = image.float()
    ...
    names = model.names
    pooler = ROIPooler(output_size=hyp['mask_resolution'], scales=(model.pooler_scale,), sampling_ratio=1,
    ...
    if pred is not None:
        ...
        for one_mask, bbox, cls, conf in zip(pred_masks_np, nbboxes, pred_cls, pred_conf):
            ...
            if not opt.nolabel:
                ...
                cv2.putText(image_display, label, (bbox[0], bbox[1] - 2), 0, opt.thickness / 3, [0, 0, 0],
...
def onImage():
    ...
    if not opt.nosave:
        ...
        cv2.imshow("finalImg", image_display)
        cv2.waitKey(0)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source', type=str, default='Street.mp4', help='source')  # image/video, 0 for webcam
    ...
    parser.add_argument('--hyp', type=str, default='data/hyp.scratch.mask.yaml',
                        help='hyperparamters file for yolov7-mask')
